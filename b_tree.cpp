/* Kode C++ untuk impelementasi B-Tree (dari berbagai sumber) */
#include <bits/stdc++.h>
using namespace std;

/* Nilai B (menyatakan ordo atau jumlah key yang diperbolehkan disimpan pada
1 node adalah N-1 sdsdsd*/
#define N 4
...
struct node *searchforleaf(struct node *root, int k, struct node *parent, int chindex)
{
    if (root)
    {
        if (root->isleaf) // cek apakah root nerupakan leaf
            return root;
...
*/
    ...
    // Cetak Tree Originaldfsdf
    cout << "Original tree: " << endl;
    for (int z = 0; z < root->n; z++)
        cout << "	" << root->key[z] << " ";
    cout << endl;
    for (int t = 0; t < N - 1; t++)
    {
    ...
    // Tambahkan Nilai 5
    cout << "Setelah menambakan lima 5: " << endl;
    root->child[0] = insert(root->child[0], 5);

    // // Cetak Tree hasil penambahan dengan elemen 5
    // for (int ii = 0; ii < root->n; ii++)
    //     cout << "	" << root->key[ii] << " ";
    // cout << endl;
    // for (int ii = 0; ii < N - 2; ii++)
    // {
    //     cout << root->child[ii]->key[0] << " ";
    //     cout << root->child[ii]->key[1] << " ";
    //     cout << root->child[ii]->key[2] << " ";
    //     cout << " ";
    // }
    // cout << endl;
    return 0;
